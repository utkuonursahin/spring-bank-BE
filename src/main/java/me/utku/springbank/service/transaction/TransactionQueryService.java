...
public class TransactionQueryService {
    ...
    public TransactionPageDto getUserAccountTransactions(User user, UUID accountId, int page, int size) {
        Page<Transaction> transactions = transactionRepository.findAllBySender_IdOrReceiver_IdAndSender_Owner_IdAndReceiver_Owner_IdOrderByCreatedAtDesc(accountId, accountId, user.getId(), user.getId(), PageRequest.of(page, size));
        return transactionMapper.toPageDto(transactions);
    }
}
